...
Ext.onReady(function() {
	...
	// 初始化Ext.QuickTips, 启动悬浮提示
	Ext.QuickTips.init();
	
	// Dummy array data for the grids
	var dummyData = [
	 ...
	 * This is usually used in conjunction with an Ext.grid.GroupingView to provide the data model for a grouped GridPanel.
	 * Internally, GroupingStore is simply a normal Store with multi sorting enabled from the start.
	 * The grouping field and direction are always injected as the first sorter pair.
	 * GroupingView picks up on the configured groupField and builds grid rows appropriately.
	 */
...
//		groupField: 'industry'
	...
	var grid = new Ext.grid.GridPanel({
		...
		columns: [
			...
			{header: 'Price', width: 20, sortable: true, dataIndex: 'price', renderer: Ext.util.Format.usMoney},
			{header: 'Change', width: 20, sortable: true, dataIndex: 'change', renderer: Ext.util.Format.usMoney},
			{header: '% Change', width: 20, sortable: true, dataIndex: 'pctChange'},
			{header: 'Industry', width: 20, sortable: true, dataIndex: 'industry'},
			{header: 'Last Updated', width: 20, sortable: true, dataIndex: 'lastChange', renderer: Ext.util.Format.dateRenderer('m/d/Y')}
		],
	...
	grid.render(document.body);		// 纯属个人意愿, 不愿出现警告标志
});
